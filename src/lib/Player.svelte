...
async function getdetail(){
    const fileCode = id; // assuming `id` is passed as a prop

   const response = await fetch(`https://api.filelions.com/api/file/info?key=1730m2zzxzv2tzii249x&file_code=${fileCode}`);
    const json = await response.json();
    const get_download = await fetch(`https://api.filelions.com/api/file/direct_link?key=1730m2zzxzv2tzii249x&file_code=${fileCode}`);
    const resdownload = await get_download.json()
    url_download = resdownload.result.versions[0].url
    fileInfo = json.result[0];
}
  async function getrelated(){
     const response = await fetch('https://api.filelions.com/api/file/list?key=1730m2zzxzv2tzii249x');
    const json = await response.json();

    const totalResults = json.result.results_total;
    const totalPages = json.result.pages;

    // Ambil 35 item secara acak
    let randomPage = Math.floor(Math.random() * totalPages) + 1;

    const randomResponse = await fetch(`https://api.filelions.com/api/file/list?key=1730m2zzxzv2tzii249x&page=${randomPage}&per_page=35`);
    const randomJson = await randomResponse.json();

    relatedVideos = randomJson.result.files;
  }
  ...
  {#each relatedVideos as video (video.thumbnail)}
    <div class="col-md-4 col-12">
      <Card class="p-1 mt-3" style="background-color: #d9d2d6;" on:click={()=>gotopage(video.file_code)}>
        ...
        <button on:click={()=>gotopage(video.file_code)} target="_blank" class="btn"
          style="background-color:orange;color: black;
          font-weight: bold;
          " 
          >Nonton Sekarang</button>
      </Card>
    </div>
  {/each}
  {/if}
